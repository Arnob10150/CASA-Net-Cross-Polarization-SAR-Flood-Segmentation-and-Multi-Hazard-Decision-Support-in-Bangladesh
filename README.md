...
- `data/raw/unosat/sylhet_2024/`: curated Sylhet June 2024 UNOSAT flood extent and analysis-extent shapefile components.
- `data/raw/hazards/erosion/labels/`: Bangladesh erosion label inputs built from HydroRIVERS corridor files.
- `data/raw/hazards/landslide/labels/`: Bangladesh landslide seed data and Bangladesh subset of the NASA Global Landslide Catalog.
- `data/processed/masks_multihazard/`: exported flood, erosion, and landslide raster masks used for benchmarking and hazard supervision.
- `data/processed/patch_manifests/`: strict and optimistic split files, normalization files, dataset info, and patch manifests; large patch arrays are intentionally excluded.
...
## Notes
- The package is designed for review and upload, not for re-running the full heavy training pipeline from scratch without re-downloading or regenerating the excluded large assets.
