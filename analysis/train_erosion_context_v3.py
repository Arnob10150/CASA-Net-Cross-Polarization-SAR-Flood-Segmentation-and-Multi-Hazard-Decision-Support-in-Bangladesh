...
TARGET_PATH = ROOT / 'data' / 'processed' / 'masks_multihazard' / 'erosion' / 'erosion_seed_mask_sylhet_20240619.tif'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(); parser.add_argument('--epochs', type=int, default=6); parser.add_argument('--batch-size', type=int, default=4); parser.add_argument('--patience', type=int, default=3); args = parser.parse_args()
    ...
    start_ckpt = MODELS_DIR / 'casa_net_erosion_seed.pth'
    ...
    model.load_state_dict(best_state); ckpt_path = MODELS_DIR / 'casa_net_erosion_context_v3.pth'; torch.save(model.state_dict(), ckpt_path)
    history_df = pd.DataFrame(history); history_path = REPORT_DIR / 'erosion_context_v3_history.csv'; history_df.to_csv(history_path, index=False)
    ...
    metrics_path = REPORT_DIR / 'test_metrics_erosion_context_v3.csv'; pd.DataFrame([test_metrics]).to_csv(metrics_path, index=False)
    fig, ax = plt.subplots(figsize=(5,4), dpi=300); sns.heatmap(confusion_matrix(y_true, (y_prob>=best_threshold).astype(np.uint8)), annot=True, fmt='d', cmap='Blues', cbar=False, ax=ax); ax.set_title('Erosion context v3 confusion matrix'); plt.tight_layout(); plt.savefig(FIGURES_DIR / 'erosion_confusion_matrix_context_v3.png', dpi=300, bbox_inches='tight'); plt.close(fig)
    precision_vals, recall_vals, _ = precision_recall_curve(y_true, y_prob); fpr, tpr, _ = roc_curve(y_true, y_prob)
    fig, axes = plt.subplots(1,2, figsize=(12,4), dpi=300); axes[0].plot(recall_vals, precision_vals); axes[0].set_title('Erosion PR context v3'); axes[1].plot(fpr, tpr, label=f'AUC={auc(fpr,tpr):.3f}'); axes[1].plot([0,1],[0,1],'--',color='gray'); axes[1].legend(); axes[1].set_title('Erosion ROC context v3'); plt.tight_layout(); plt.savefig(FIGURES_DIR / 'erosion_pr_roc_context_v3.png', dpi=300, bbox_inches='tight'); plt.close(fig)
    ...
    (REPORT_DIR / 'erosion_context_v3_summary.json').write_text(json.dumps(summary, indent=2), encoding='utf-8'); print(json.dumps(summary, indent=2))
