...
for name, model in benchmark_variants.items():
    ...
    hist_df.to_csv(REPORT_DIR / f'{hist_name}_history_optimistic.csv', index=False)
    model.load_state_dict(best_state)
    model = model.to(DEVICE)
    ckpt_name = 'casa_net_best_optimistic.pth' if name == 'CASA-Net' else f'benchmark_{hist_name}_optimistic.pth'
...
if casa_true is not None:
    ...
    ax.set_title('CASA-Net confusion matrix (optimistic overlap run)')
...
(REPORT_DIR / 'training_execution_summary_optimistic.json').write_text(json.dumps(execution, indent=2))
