...
benchmark_variants = {
    'CASA-Net': CASANet(dropout_p=0.1),
}
...
for name, model in benchmark_variants.items():
    ...
    hist_df.to_csv(REPORT_DIR / 'casanet_history_optimistic.csv', index=False)
    model.load_state_dict(best_state)
    model = model.to(DEVICE)
    ckpt_name = 'casa_net_best_optimistic.pth'
    ...
    if name == 'CASA-Net':
        ...
        pd.DataFrame([{**metrics, 'Threshold': best_threshold}]).to_csv(REPORT_DIR / 'test_metrics_casa_net_optimistic.csv', index=False)
    del model
...
benchmark_df.to_csv(REPORT_DIR / 'benchmark_results_casanet_only_optimistic.csv', index=False)
...
ax.legend()
plt.tight_layout()
plt.savefig(FIGURES_DIR / 'benchmark_model_comparison_casanet_only_optimistic.png', dpi=300, bbox_inches='tight')
plt.close(fig)
...
plt.tight_layout()
plt.savefig(FIGURES_DIR / 'benchmark_ranked_leaderboard_casanet_only_optimistic.png', dpi=300, bbox_inches='tight')
plt.close(fig)
...
if casa_true is not None:
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'confusion_matrix_casa_net_optimistic.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'precision_recall_roc_casa_net_optimistic.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
...
(REPORT_DIR / 'training_execution_summary_casanet_optimistic.json').write_text(json.dumps(execution, indent=2))
