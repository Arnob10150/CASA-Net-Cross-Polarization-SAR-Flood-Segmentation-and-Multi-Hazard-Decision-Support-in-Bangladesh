...
specs = [
    ...
    (
        ...
        MODELS_DIR / "casa_net_best_optimistic_tuned.pth",
        REPORT_DIR / "casanet_history_optimistic_tuned.csv",
    ),
]
...
benchmark_df.to_csv(REPORT_DIR / "benchmark_results_optimistic_tuned.csv", index=False)
...
ax.legend()
plt.tight_layout()
plt.savefig(FIGURES_DIR / "benchmark_model_comparison_optimistic_tuned.png", dpi=300, bbox_inches="tight")
plt.close(fig)
...
plt.tight_layout()
plt.savefig(FIGURES_DIR / "benchmark_ranked_leaderboard_optimistic_tuned.png", dpi=300, bbox_inches="tight")
plt.close(fig)
...
(REPORT_DIR / "real_cuda_benchmark_summary_optimistic_tuned.md").write_text("\n".join(summary), encoding="utf-8")
