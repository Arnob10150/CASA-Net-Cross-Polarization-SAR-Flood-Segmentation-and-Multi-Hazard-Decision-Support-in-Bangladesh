...
class HazardV2Dataset(Dataset):
    def __init__(self, hazard='erosion', split='train', augment=False):
        ...
        if hazard == 'erosion':
            self.mask_path = ROOT / 'data' / 'processed' / 'masks_multihazard' / 'erosion' / 'erosion_seed_mask_sylhet_20240619_v2.tif'
            self.context_path = ROOT / 'data' / 'processed' / 'masks_multihazard' / 'erosion' / 'erosion_seed_mask_sylhet_20240619.tif'
            self.start_ckpt = MODELS_DIR / 'casa_net_erosion_seed.pth'
        elif hazard == 'landslide':
            self.mask_path = ROOT / 'data' / 'processed' / 'masks_multihazard' / 'landslide' / 'landslide_seed_mask_sylhet_20240619_v2.tif'
            self.context_path = None
            self.start_ckpt = MODELS_DIR / 'casa_net_landslide_seed.pth'
        else:
        ...
        for path in self.paths:
            ...
            mask = self.mask_src.read(1, window=Window(col, row, 256, 256))
            self.positive_px.append(float(mask.sum()))

    def __len__(self):
        return len(self.paths)

    def make_terrain(self, terrain, row, col):
        slope = terrain[0]
        twi = terrain[1]
        hand = terrain[3]
        if self.hazard == 'erosion':
            corridor = self.context_src.read(1, window=Window(col, row, 256, 256)).astype(np.float32)
            return np.stack([slope, twi, hand, corridor], axis=0).astype(np.float32)
        rough = ndi.uniform_filter(slope, size=5)
        rough2 = ndi.uniform_filter(slope * slope, size=5)
        rough = np.sqrt(np.clip(rough2 - rough * rough, 0, None)).astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        terrain = self.make_terrain(terrain, row, col)
        mask = self.mask_src.read(1, window=Window(col, row, 256, 256)).astype(np.float32)[None, ...]
        if self.augment:
...
def sweep_thresholds(y_true, y_prob):
    rows = []
    for th in np.arange(0.25, 0.86, 0.05):
...
class ResNet34Encoder(nn.Module):
    def __init__(self):
        ...
        old_weight = torch.randn_like(backbone.conv1.weight)
...
class MobileNetV3Encoder(nn.Module):
    def __init__(self):
        ...
        old_weight = torch.randn_like(backbone.features[0][0].weight)
...
class UNetDecoder(nn.Module):
    ...
    def forward(self, fused, film_params):
        x = self.blocks[0](fused[-1], fused[-2]); x = apply_film(x, film_params[0])
        x = self.blocks[1](x, fused[-3]); x = apply_film(x, film_params[1])
        x = self.blocks[2](x, fused[-4]); x = apply_film(x, film_params[2])
...
if __name__ == '__main__':
    ...
    train_ds = HazardV2Dataset(args.hazard, 'train', augment=True)
    val_ds = HazardV2Dataset(args.hazard, 'val', augment=False)
    test_ds = HazardV2Dataset(args.hazard, 'test', augment=False)
    ...
    start_ckpt = train_ds.start_ckpt if train_ds.start_ckpt.exists() else (MODELS_DIR / 'casa_net_best_v3.pth')
    ...
    ckpt_path = MODELS_DIR / f'casa_net_{args.hazard}_v2.pth'
    torch.save(model.state_dict(), ckpt_path)

    history_df = pd.DataFrame(history)
    history_path = REPORT_DIR / f'{args.hazard}_casanet_v2_history.csv'
    ...
    test_metrics['supervision_type'] = 'bangladesh_preferred_v2' if args.hazard == 'erosion' else 'bangladesh_preferred_v2'
    metrics_path = REPORT_DIR / f'test_metrics_{args.hazard}_casa_net_v2.csv'
    pd.DataFrame([test_metrics]).to_csv(metrics_path, index=False)

    sweep_df = pd.DataFrame([{'threshold': th, **metrics_from_prob(y_true, y_prob, th)} for th in np.arange(0.25, 0.86, 0.05)])
    sweep_df.to_csv(REPORT_DIR / f'{args.hazard}_threshold_sweep_v2.csv', index=False)

    cm = confusion_matrix(y_true, (y_prob >= best_threshold).astype(np.uint8))
    fig, ax = plt.subplots(figsize=(5, 4), dpi=300)
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar=False, ax=ax)
    ax.set_title(f'{args.hazard.title()} CASA-Net v2 confusion matrix')
    ax.set_xlabel('Predicted')
    ax.set_ylabel('Actual')
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / f'{args.hazard}_confusion_matrix_v2.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
    ...
    axes[0].set_title(f'{args.hazard.title()} Precision-Recall v2')
    axes[0].set_xlabel('Recall')
    axes[0].set_ylabel('Precision')
    axes[1].plot(fpr, tpr, color='#e76f51', linewidth=2, label=f"AUC={auc(fpr, tpr):.3f}")
    axes[1].plot([0, 1], [0, 1], '--', color='gray')
    axes[1].legend()
    axes[1].set_title(f'{args.hazard.title()} ROC v2')
    axes[1].set_xlabel('False positive rate')
    axes[1].set_ylabel('True positive rate')
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / f'{args.hazard}_pr_roc_v2.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
    ...
    ax.set_title(f'{args.hazard.title()} training history v2')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax2.set_ylabel('IoU')
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / f'{args.hazard}_loss_iou_curve_v2.png', dpi=300, bbox_inches='tight')
    plt.close(fig)
    ...
    (REPORT_DIR / f'{args.hazard}_training_summary_v2.json').write_text(json.dumps(summary, indent=2), encoding='utf-8')
